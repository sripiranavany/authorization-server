...
public class ApiAuthorizationController {
    ...
    public ResponseEntity<?> authorize(@Valid @RequestBody AuthorizationRequest request) {
        auditLogger.info("OAuth2 Authorization Request - Client: {}, Username: {}, Scopes: {}",
                request.getClientId(), request.getUsername(), request.getScope());
    ...
    public ResponseEntity<?> exchangeToken(@Valid @RequestBody TokenExchangeRequest request) {
        auditLogger.info("OAuth2 Token Exchange Request - Client: {}, Grant Type: {}, Code: {}",
                request.getClientId(), request.getGrantType(), request.getCode() != null ? "***" : "null");
        ...
        try {
            ...
            OAuth2Token accessToken = new OAuth2Token() {
                ...
                public Instant getExpiresAt() {
                    return Instant.now().plus(client.getTokenSettings().getAccessTokenTimeToLive());
                }
            };
    ...
    public ResponseEntity<?> refreshToken(@Valid @RequestBody RefreshTokenRequest request) {
        auditLogger.info("OAuth2 Refresh Token Request - Client: {}, Grant Type: {}",
    ...
    public ResponseEntity<?> introspectRefreshToken(@Valid @RequestBody RefreshTokenIntrospectRequest request) {
        auditLogger.info("OAuth2 Refresh Token Introspection Request - Client: {}", request.getClientId());
    ...
    public ResponseEntity<?> getTokenStatistics() {
        try {
            ...
            Map<String, Object> response = new HashMap<>();
            Map<String, Object> authCodesMap = new HashMap<>();
            authCodesMap.put("active", stats.getActiveAuthorizationCodes());
            authCodesMap.put("expired", stats.getExpiredAuthorizationCodes());
            authCodesMap.put("total", stats.getActiveAuthorizationCodes() + stats.getExpiredAuthorizationCodes());
            response.put("authorization_codes", authCodesMap);

            Map<String, Object> refreshTokensMap = new HashMap<>();
            refreshTokensMap.put("active", stats.getActiveRefreshTokens());
            refreshTokensMap.put("expired", stats.getExpiredRefreshTokens());
            refreshTokensMap.put("used", stats.getUsedRefreshTokens());
            refreshTokensMap.put("total", stats.getActiveRefreshTokens() + stats.getExpiredRefreshTokens() + stats.getUsedRefreshTokens());
            response.put("refresh_tokens", refreshTokensMap);
